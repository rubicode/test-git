...
class DataItem extends Component{
  ...
  handleSave(){
    ...
    console.log(this.props.data.id, name, phone);
    this.setState({editing: false})
  }

  render(){
    const {data, deletePhoneBook, editPhoneBook} = this.props
    if(this.state.editing){
      ...
      return(
        ...
        <button type="button" className="delete-btn btn btn-danger" onClick={() => deletePhoneBook(data.id)}><span className="glyphicon glyphicon-trash"></span> delete</button>
        </td>
        </tr>
      )
    }
